"""
Модуль логування для системи заявок
...
class BotLogger:
    ...
    def __init__(self, log_file: str = "logs.txt", log_level: str = "INFO", use_db: bool = True):
        ...
        self.use_db = use_db
        self.logger = logging.getLogger("tickets_system")
    ...
    def log_ticket_created(self, user_id: int, ticket_id: int, ticket_type: str) -> None:
        """Логування створення заявки"""
        message = f"UserID: {user_id} | Створено заявку ID: {ticket_id} | Тип: {ticket_type}"
        self.logger.info(message)
        self._save_to_db('INFO', message, user_id, 'ticket_created')
    
    def log_ticket_status_changed(self, admin_id: int, ticket_id: int, old_status: str, new_status: str) -> None:
        """Логування зміни статусу заявки"""
        message = f"AdminID: {admin_id} | Заявка ID: {ticket_id} | Статус: {old_status} → {new_status}"
        self.logger.info(message)
        self._save_to_db('INFO', message, admin_id, 'ticket_status_changed')
