...
Створення опитувань через Telegram та опрацювання результатів адміном
"""
import os
import json
import requests
...
class PollManager:
    ...
    def create_poll(
        ...
        Args:
            ...
            expires_at: Термін дії опитування (опціонально)
    ...
    def send_poll_to_users(self, poll_id: int, user_ids: Optional[List[int]] = None) -> Dict[str, int]:
        ...
        Args:
            poll_id: ID опитування
            user_ids: Список ID користувачів для відправки. Якщо None - відправляється всім користувачам з увімкненими оповіщеннями
